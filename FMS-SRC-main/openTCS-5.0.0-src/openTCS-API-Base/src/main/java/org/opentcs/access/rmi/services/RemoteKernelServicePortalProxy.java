...
public class RemoteKernelServicePortalProxy
  ...
  @Override
  public void login(@Nonnull String hostName, int port, String username, String password)
    ...
    try {
      ...
      setClientId(getRemoteService().login(username, password, Locale.getDefault(), eventFilter));
  ...
  public void login(@Nonnull String hostName, int port)
      throws CredentialsException, ServiceUnavailableException {
    this.login(hostName, port, userName, password);
  }
