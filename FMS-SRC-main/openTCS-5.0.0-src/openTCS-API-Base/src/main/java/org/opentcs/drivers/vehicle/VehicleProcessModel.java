...
public class VehicleProcessModel {
  ...
  /**
   * The vehicle's voltage
   * customized by Khoi - AUBOT
   */
  private float voltage = 0;
  /**
   * The vehicles electric current
   * customized by Khoi - AUBOT
   */
  private float current = 0;
  /**
   * Vehicle distance from last position
   */
  ...
  /**
   * Current working route of vehicle
   */
  ...
  public void setVehiclePrecisePositionByDistance(double distance) {
    double oldValue = this.distance;
    this.distance = distance;

    getPropertyChangeSupport().firePropertyChange(VehicleProcessModel.Attribute.PRECISE_POSITION_DISTANCE.name(),
            oldValue,
            distance);
  }
  ...
  public void setVehicleErrorCode(int errorCode) {
    int oldValue = this.errorCode;
    this.errorCode = errorCode;

    getPropertyChangeSupport().firePropertyChange(VehicleProcessModel.Attribute.ERROR_CODE.name(),
            oldValue,
            errorCode);
  }
  ...
  public void setVehicleErrorMessages(HashMap<String, String> errorMessage) {
    HashMap<String, String> oldValue = this.errorMessage;
    this.errorMessage = errorMessage;

    getPropertyChangeSupport().firePropertyChange(VehicleProcessModel.Attribute.ERROR_MESSAGES.name(),
            oldValue,
            errorMessage);
  }
  ...
  public float getVehicleVoltage() {
    return this.voltage;
  }

  public void setVehicleVoltage(float voltage) {
    float oldValue = this.voltage;
    this.voltage = voltage;

    getPropertyChangeSupport().firePropertyChange(Attribute.VOLTAGE.name(),
            oldValue,
            voltage);
  }

  public float getVehicleCurrent() {
    return this.current;
  }

  public void setVehicleCurrent(float current) {
    float oldValue = this.current;
    this.current = current;

    getPropertyChangeSupport().firePropertyChange(Attribute.CURRENT.name(),
            oldValue,
            current);
  }
  ...
  public void setVehicleWarningState(boolean warning) {
    boolean oldValue = this.warning;
    this.warning = warning;

    getPropertyChangeSupport().firePropertyChange(Attribute.IS_WARNING.name(),
            oldValue,
            warning);
  }

  public WorkingRoute.WorkingRouteRaw getWorkingRoute() {
    return workingRoute;
  }

  public void setVehicleCurrentRoute(WorkingRoute.WorkingRouteRaw workingRoute) {
    WorkingRoute.WorkingRouteRaw oldValue = this.workingRoute;
    if(oldValue != null && oldValue.equals(workingRoute)){
      getPropertyChangeSupport().firePropertyChange(Attribute.DUPLICATE_ROUTE.name(),
              oldValue,
              new WorkingRoute.WorkingRouteRaw(oldValue.getMapId()));
    }else{
      getPropertyChangeSupport().firePropertyChange(Attribute.CURRENT_ROUTE.name(),
              oldValue,
              workingRoute);
  ...
  public void syncRoutesSuccess(int mapId) {
    getPropertyChangeSupport().firePropertyChange(Attribute.SYNC_ROUTES_SUCCESS.name(), null, mapId);
  }

  public void syncRoutesFailed(String errorString) {
    getPropertyChangeSupport().firePropertyChange(Attribute.SYNC_ROUTES_FAILED.name(), null, errorString);
  }
  ...
  public enum Attribute {
    ...
    /**
     * Indicates syncing route success
     */
    SYNC_ROUTES_SUCCESS,
    /**
     * Indicates syncing route failed
     */
    SYNC_ROUTES_FAILED,
    /**
     * Indicates an answer from vehicle for asking route
     */
    CURRENT_ROUTE,
