...
public class TransportOrder
  ...
  public TransportOrder(String name, List<DriveOrder> driveOrders, boolean isLoop) {
    super(name,
          new HashMap<>(),
          new ObjectHistory().withEntryAppended(new ObjectHistory.Entry(ORDER_CREATED)));
    this.type = OrderConstants.TYPE_NONE;
  ...
  private TransportOrder(String name,
                         ...
                         boolean isLoop, String startPosition) {
    super(name, properties, history);
    this.type = requireNonNull(type, "type");
    requireNonNull(driveOrders, "driveOrders");
    this.driveOrders = new LinkedList<>();
    for (DriveOrder driveOrder : driveOrders) {
      DriveOrder newDriveOrder = driveOrder.withTransportOrder(this.getReference());
      newDriveOrder.setCurrentCountOnSteps(driveOrder.getCurrentCountOnSteps());
      this.driveOrders.add(newDriveOrder);
    }
  ...
  public TransportOrder withProperty(String key, String value) {
    return new TransportOrder(getName(),
                              ...
                              processingVehicle,
                              state,
                              finishedTime, isLoop, startPosition);
  }
  ...
  public TransportOrder withState(@Nonnull State state) {
    ...
    return new TransportOrder(getName(),
                              ...
                              processingVehicle,
                              state,
                              state == State.FINISHED ? Instant.now() : finishedTime, isLoop, startPosition);
  }
  ...
  public TransportOrder withStartPosition(String startPosition) {
    return new TransportOrder(getName(),
            getProperties(),
            getHistory(),
            type,
