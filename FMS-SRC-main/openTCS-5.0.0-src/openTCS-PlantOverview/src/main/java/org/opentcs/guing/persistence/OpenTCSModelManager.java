...
public class OpenTCSModelManager
  ...
  @Override
  public void addVehicle(Vehicle vehicle, KernelServicePortal portal) {
    TCSObjectService objectService = (TCSObjectService) portal.getPlantModelService();
    restoreModelVehicle(vehicle, systemModel, objectService);
  }

  @Override
  public void removeVehicle(Vehicle vehicle, KernelServicePortal portal) {
  ...
  private void restoreModelGroups(Set<Group> allGroups, SystemModel systemModel,
    ...
    for (Group group : allGroups) {
      ...
      ModelLayoutElement element = systemModel.getLayoutMap().get(group.getReference());
      procAdapterUtil.processAdapterFor(groupModel)
          .updateModelProperties(group, groupModel, systemModel, objectService, element);
  ...
  private LabeledLocationFigure createLocationFigure(LocationModel locationModel,
    ...
    int fWidth = (int) Math.round((Double) locationModel.getWidthProperty().getValue());
    int fHeight = (int) Math.round((Double) locationModel.getHeightProperty().getValue());

    locationFigure.setfWidth(fWidth);
    locationFigure.setfHeight(fHeight);
    locationFigure.setDisplayBoxSize(fWidth,fHeight);
...
//      String labelOrientationAngle = layoutProperties.get(ElementPropKeys.POINT_LABEL_ORIENTATION_ANGLE);
  ...
  private void restoreModelVehicle(Vehicle vehicle, SystemModel systemModel, TCSObjectService objectService) {
