...
/**
 * Provides a {@link SharedKernelServicePortal} for clients in the kernel control center application.
 *
...
public class ApplicationPortalProvider
  ...
  public SharedKernelServicePortal register()
    ...
    if (!portalShared()) {
      ...
      if (!portalManager.connect(1)) {
        throw new ServiceUnavailableException("Connection canceled");
      }
    }
    clients.add(token);
