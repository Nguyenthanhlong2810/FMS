...
import org.opentcs.guing.components.dockable.theme.CAubotTheme;
...
public class DockingManager {
  ...
  private static final ImageIcon DOCK_ICON = ImageDirectory.getImageIcon("/dock/dockable.png");
  private static final ImageIcon IMPORTANT_ICON = ImageDirectory.getImageIcon("/dock/important.png");
  /**
  ...
  public static final String VEHICLE_STATE_TABLE_ID = "vehicle_state_table_dock";
  ...
  public DefaultSingleCDockable createDockable(String id,
    ...
    dockable.addCDockableLocationListener(e -> {
      if (e.getNewShowing()) {
        dockable.setTitleIcon(DOCK_ICON);
      }
    });
  ...
  public void reset() {
    ...
    removeDockable(VEHICLE_STATE_TABLE_ID);
  ...
  public void initializeDockables(JFrame frame,
    ...
    Objects.requireNonNull(vehicleStateTable,"vehicleStateTable is null");
    ...
    control.getController().setTheme(new CAubotTheme(control));
    ...
    DefaultSingleCDockable vehiclesDock
    ...
    DefaultSingleCDockable vehicleStateDock
        = createDockable(VEHICLE_STATE_TABLE_ID,
                          bundleTreeView.getString("dockingManager.panel_tables.title"),
                            vehicleStateTable,
                            false);
    ...
    DefaultSingleCDockable treeGroups
        = createDockable(GROUPS_ID,
                         ...
                         false);
    DefaultSingleCDockable propertiesDock
    ...
    DefaultSingleCDockable kernelMassages = createDockable(STATUS_ID,
                          ResourceBundleUtil.getBundle(I18nPlantOverview.STATUS_PATH)
                                  .getString("dockingManager.panel_status.title"),
                          statusScrollPane,
                          false);

//    grid.add(1300, 0, 300, 500, treeTabPane);
    grid.add(0, 0, 1, 5, treeGroups);
    grid.add(0, 0, 1, 5, treeBlocks);
    grid.add(0, 0, 1, 5, treeViewDock);
    grid.add(0, 0, 1, 5, vehiclesDock);
    grid.add(8, 0, 2, 10, propertiesDock);
    grid.add(3, 0, 8, 4, courseTabPane);
    grid.add(3, 4, 7, 1, kernelMassages);
    grid.add(3, 4, 7, 1, vehicleStateDock);
...
//    addTabTo(kernelMassages, TREE_TAB_PANE_ID, 3);
//    treeTabPane.getStation().setFrontDockable(treeViewDock.intern());
  ...
  public void markDockableImportant(String id) {
    SingleCDockable dockable = control.getSingleDockable(id);
    if (!dockable.isShowing()) {
      ((DefaultSingleCDockable) dockable).setTitleIcon(IMPORTANT_ICON);
    }
  }
}
