...
public class OpenTCSDrawingView
  ...
  private GroupModel fFocusGroup;

  private boolean groupAlwaysFocus;
  /**
...
//        AbstractConnectableModelComponent abstractFigure = (AbstractConnectableModelComponent) model;
        ...
        AbstractFigure figure = (AbstractFigure) modelManager.getModel().getFigure(model);
...
//    if (RouteDialog.isRouteActive()) {
//      drawPathDecoration(g2d,
//                         driveOrderFigures(RouteDialog.getRoutePaths()),
//                         RouteDialog.getVehicle().getDriveOrderColor(),
...
//    }
  ...
  private void highlightFocus(Graphics2D g2d) {
    ...
    Color[] colors = {
      ...
      new Color(0.9f, 0.9f, 0.9f, 0.25f) // Background
    };
  ...
  public void focusGroup(GroupModel groupModel, int timeout) {
    fFocusGroup = groupModel;
    groupAlwaysFocus = timeout < 0;
    repaintDrawingArea();
    final GroupModel currentFocusGroup = groupModel;
    ...
    new Thread(() -> {
      ...
      // prevents repainting of the drawing area if in the 3 second wait
      // time another figure was selected
      if (currentFocusGroup == fFocusGroup && !groupAlwaysFocus) {
        fFocusGroup = null;
        repaintDrawingArea();
      }
  ...
  public void removeFocusGroup() {
    fFocusGroup = null;
    groupAlwaysFocus = false;
  }

  private void highlightRoute(Graphics2D g2d) {
    final GroupModel focus = fFocusGroup;
    List<Figure> children = getDrawing().getChildren();
    List<ModelComponent> components = focus.getChildComponents();
    List<Figure> figures = new ArrayList<>(components.size());
    List<ModelComponent> connections = components.stream().filter(c -> c instanceof PathModel).collect(Collectors.toList());
    ...
    for (Figure figure : children) {
      ...
      if (connections.contains(model)) {
        figures.add(figure);
      }
    }

    drawPathDecoration(g2d, figures, focus.getPropertyColor().getColor(), Strokes.PATH_ON_ROUTE);
  }
  ...
  private void drawRoutes(Graphics2D g2d) {
    List<Figure> children = getDrawing().getChildren();
    for (GroupModel groupModel : modelManager.getModel().getGroupModels()) {
      List<ModelComponent> components = groupModel.getChildComponents();
//      List<ModelComponent> connections = new ArrayList<>();
//      for (int i = 0; i < components.size(); i++) {
//        PointModel start = (PointModel) components.get(i);
//        PointModel end = (PointModel) components.get(i + 1 == components.size() ? 0 : i + 1 );
//        if (start.hasConnectionTo(end)) {
//          connections.add(start.getConnectionTo(end));
//        }
//
//      }
      List<Figure> figures = new ArrayList<>(components.size());
      ...
      drawPathDecoration(g2d, figures, groupModel.getPropertyColor().getColor(), Strokes.PATH_ON_ROUTE);
    }
  }
  ...
  private void deleteSelectedFigures() {
    ...
    Set<PointModel> deletedPoints = new HashSet<>();
    Set<PathModel> deletedPaths = new HashSet<>();
    ...
    for(Figure figure : deletedFigures){
      if(figure instanceof PathConnection){
        deletedPaths.add(((PathConnection) figure).getModel());
      }else if(figure instanceof LabeledPointFigure){
        PointModel point = ((LabeledPointFigure) figure).getPresentationFigure().getModel();
        deletedPoints.add(point);
        point.getConnections().forEach(connection -> deletedPaths.add((PathModel) connection));
      }
    }

    //delete related route
    List<GroupModel> groupModels = modelManager.getModel().getGroupModels();
    Set<GroupModel> deletedGroup = groupModels.stream()
            .filter(group -> group.getChildComponents().stream().anyMatch(deletedPaths::contains))
            .collect(Collectors.toSet());

    if(deletedFigures.isEmpty()){
      return;
    }
    if (!deletedGroup.isEmpty()) {
      UserMessageHelper.ReturnType returnType = userMessageHelper.showConfirmDialog("Delete","Do you want to delete this object ?" +
              "\n This maybe delete the related items"+ deletedGroup, UserMessageHelper.Type.QUESTION);
    ...
    List<PointModel> pointModels = modelManager.getModel().getPointModels();
    for (PointModel deletePointName : deletedPoints) {
      for (PointModel pointModel : pointModels) {
        if (pointModel.getLeftPoint().getValue().equals(deletePointName.getName())) {
          pointModel.getLeftPoint().setValue("");
        }else if(pointModel.getRightPoint().getValue().equals(deletePointName.getName())){
          pointModel.getRightPoint().setValue("");
        }
      }
    }

    for(GroupModel groupModel : deletedGroup){
      openTCSView.deleteGroup(groupModel);
    }
  }
