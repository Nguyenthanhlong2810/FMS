...
public class ModelValidator {
  ...
  private boolean validatePoint(SystemModel model, PointModel point) {
    ...
    List<AbstractConnection> outgoingConnections = point.getConnections().stream()
            .filter(connection -> Objects.equals(connection.getStartComponent(), point))
            .collect(Collectors.toList());
    if (outgoingConnections.size() >= 2) {
      if (Strings.isNullOrEmpty(point.getLeftPoint().toString())
       || Strings.isNullOrEmpty(point.getRightPoint().toString())) {
        errorOccurred(point, "modelValidator.error_directionPointsIsNotSet.text");
        valid = false;
      } else {
        SelectionPointProperty<String> leftPoint = point.getLeftPoint();
        if (model.getPointModel(leftPoint.toString()) == null) {
          errorOccurred(point, "modelValidator.error_leftPointNotExisting.text", leftPoint.getValue());
          valid = false;
        }

        SelectionPointProperty<String> rightPoint = point.getRightPoint();
        if (model.getPointModel(rightPoint.toString()) == null) {
          errorOccurred(point, "modelValidator.error_rightPointNotExisting.text", rightPoint.getValue());
          valid = false;
        }
      }
    }
