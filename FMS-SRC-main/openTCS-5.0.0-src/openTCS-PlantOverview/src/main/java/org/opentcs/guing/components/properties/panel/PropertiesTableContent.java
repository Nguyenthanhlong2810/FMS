...
import org.opentcs.guing.components.properties.table.*;
...
public class PropertiesTableContent
  ...
  public void tableModelChanged() {
    ...
    } else if(fModel instanceof GroupModel){
//      openTCSView.highlightRoute((GroupModel) fModel,((GroupModel) fModel).isHighlight());
      for(OpenTCSDrawingView drawingView : openTCSView.getDrawingViews()){
...
//    }
  ...
  protected void setTableCellRenderers() {
    ...
    fTable.setDefaultRenderer(PointProperty.class, new PointPropertyCellRenderer());
  }
  ...
  protected void setTableCellEditors() {
    ...
    //Point left
    content = new SelectionPointEditorPanel(defaultRenderer);
    dialog = new StandardDetailsDialog(dialogParent, true, content);
    undoableEditor
            = new UndoableCellEditor(new SelectionPointCellEditor(new JComboBox<>(), umh,modelManager));
    undoableEditor.setDetailsDialog(dialog);
    undoableEditor.setUndoManager(fUndoRedoManager);
    fCellEditors.add(undoableEditor);
    fTable.setDefaultEditor(SelectionPointProperty.class, undoableEditor);

    // Point property
    ListCellRenderer<String> pointRenderers
            = new StringListCellRenderer<>(this::objectToString);
    content = new SelectionPointEditorPanel(pointTypeRenderer);
    dialog = new StandardDetailsDialog(dialogParent, true, content);
    JComboBox<String> pointComboBoxs = new JComboBox<>();
    pointComboBoxs.setRenderer(pointRenderers);
    undoableEditor
            = new UndoableCellEditor(new SelectionPointCellEditor(new JComboBox<>(), umh,modelManager));
    undoableEditor.setDetailsDialog(dialog);
    undoableEditor.setUndoManager(fUndoRedoManager);
    fCellEditors.add(undoableEditor);
    fTable.setDefaultEditor(PointProperty.class, undoableEditor);
