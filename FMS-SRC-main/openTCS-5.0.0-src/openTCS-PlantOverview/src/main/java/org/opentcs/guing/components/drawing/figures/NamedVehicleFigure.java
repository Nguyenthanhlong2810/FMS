...
public class NamedVehicleFigure
  ...
  protected void drawFill(Graphics2D g2d) {
    ...
    WorkingRoute workingRoute = getModel().getVehicle().getWorkingRoute();
    if (workingRoute.haveRoute()) {
      GroupModel route = getModelManager().getModel().getGroupModel(workingRoute.getCurrentGroup().getName());
      g2d.setPaint(route.getPropertyColor().getColor());
    }
  ...
  protected void updateFigureDetails(VehicleModel model) {
    ...
    //Vehicle will use its theme after model is presisted instead of injected theme - customized by Khoi - Aubot
    fImage = model.getVehicleTheme().updateMaps(model.getPropertyTypeTheme().getText())
                                                      .getImage(model.getVehicle());
    //
