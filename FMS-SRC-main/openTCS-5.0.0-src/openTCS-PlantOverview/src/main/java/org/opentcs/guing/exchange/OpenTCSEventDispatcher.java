...
public class OpenTCSEventDispatcher
  ...
  public void onEvent(Object event) {
    if (event instanceof TCSObjectEvent) {
      ...
      handleUserNotification(((NotificationPublicationEvent) event).getNotification());
  ...
  private void handleUserNotification(UserNotification notification) {
    if (notification instanceof VehicleNotification) {
      VehicleNotification vn = (VehicleNotification) notification;
      String message;
      if (vn.getState() == VehicleNotification.State.WRONG_NEXT_POSITION) {
        message = VEHICLE_STATE_BUNDLE.getFormatted("vehicleNotification.message.wrongNextPosition",
                vn.getPreviousVehicle().getNextPosition().getName(),
                vn.getVehicle().getCurrentPosition().getName());
        messageDisplay.display(new UserNotification(vn.getSource(), message, UserNotification.Level.NOTEWORTHY));
      } else if (vn.getState() == VehicleNotification.State.LOST_WORKING_ROUTE) {
        message = VEHICLE_STATE_BUNDLE.getFormatted("vehicleNotification.message.lostWorkingRoute",
                vn.getVehicle().getWorkingRoute().getCurrentGroup().getName());
        messageDisplay.display(new UserNotification(vn.getSource(), message, UserNotification.Level.IMPORTANT));
      }
    } else {
      messageDisplay.display(notification);
    }
  }
}
