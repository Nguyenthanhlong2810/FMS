...
//    final JCheckBoxMenuItem cbHighlight = new JCheckBoxMenuItem(
//            "Highlight", groupFolder.isHighlight());
//    cbHighlight.addActionListener((ActionEvent e)
//            -> setRouteHighlight(groupFolder,cbHighlight.isSelected(),openTCSView));
//    menu.add(cbHighlight);
//
//    menu.addSeparator();
...
//    menu.add(item);
    ...
    JMenuItem item = new JMenuItem(labels.getString("groupsMouseAdapter.popupMenuItem_deleteGroup.text"));
    item.addActionListener((ActionEvent e) -> openTCSView.deleteGroup(groupFolder));

    menu.add(item);
    menu.show(treeView.getTree(), x, y);
  }
//  public void setRouteHighlight(GroupModel gm, boolean highlight,OpenTCSView openTCSView) {
//    gm.setIsHighlight(highlight);
////    openTCSView.highlightRoute(gm, gm.isHighlight());
//    openTCSView.getDrawingViews().forEach(drawingView -> drawingView.focusGroup(gm));
//  }
  /**
