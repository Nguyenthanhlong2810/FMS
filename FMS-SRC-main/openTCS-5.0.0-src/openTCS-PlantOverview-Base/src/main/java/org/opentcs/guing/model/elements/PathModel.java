...
public class PathModel
  ...
  private void createProperties() {
    ...
    LinerTypeProperty pPathConnType = new LinerTypeProperty(this, Arrays.asList(Type.DIRECT, Type.BEZIER, Type.BEZIER_3), Type.DIRECT);
  ...
  public void propertiesChanged(AttributesChangeEvent e) {
    if (getStartComponent().getPropertyName().hasChanged()
      ...
      ModelComponent start = getStartComponent();
      ModelComponent end = getEndComponent();
      if (start instanceof PointModel && end instanceof PointModel) {
        String endOldName = getPropertyEndComponent().getText();
        if (((PointModel) start).getLeftPoint().getValue().equals(endOldName)) {
          ((PointModel) start).getLeftPoint().setValue(end.getName());
          ((PointModel) start).getLeftPoint().markChanged();
        } else if (((PointModel) start).getRightPoint().getValue().equals(endOldName)) {
          ((PointModel) start).getRightPoint().setValue(end.getName());
          ((PointModel) start).getRightPoint().markChanged();
        }
      }
    }
