...
public class KernelControlCenter
  ...
  public KernelControlCenter(
    ...
    initComponents();
    initLanguageMenu();
    setIconImages(Icons.getImages());
    aboutDialog = new AboutDialog(this, false);
    aboutDialog.setAlwaysOnTop(true);
  }

  private void initLanguageMenu() {
    String[] locales = {"en", "de", "vi"};
    for (String locale : locales) {
      menuLanguage.add(new AbstractAction(locale) {
        @Override
        public void actionPerformed(ActionEvent e) {
          if (JOptionPane.showConfirmDialog(null,
                  BUNDLE.getString("languageMenu.confirmText") + " " + locale,
                  BUNDLE.getString("languageMenu.confirm"),
                  JOptionPane.YES_NO_OPTION)
                  != JOptionPane.OK_OPTION) {
            return;
          }
          final String filePath = "config/opentcs-kernelcontrolcenter-defaults-baseline.properties";
          ...
          try {
            ...
            properties.setProperty("kernelcontrolcenter.locale", locale);

            outputStream = new FileOutputStream(filePath);
            properties.store(outputStream, "Modify locate");
            JOptionPane.showMessageDialog(null, BUNDLE.getString("languageMenu.changeLanguage"));
  ...
  public void terminate() {
    ...
    setVisible(false);
    dispose();
    String path = KernelControlCenter.class.getCanonicalName() + ".terminate()";
    String content = "KernelControlCenter Shutdown";
    writeLog(path,content,Logs.LEVEL.DEBUG);
    initialized = false;
  }
  ...
  private void initComponents() {
    ...
    menuLanguage.setText(bundle.getString("languageMenu.text")); // NOI18N
