...
public class StandardRemotePlantModelService
  ...
  @Override
  public void createPlantModel(ClientID clientId, PlantModelCreationTO to, boolean isPersistFromRemote) {
    userManager.verifyCredentials(clientId, UserPermission.MODIFY_MODEL);

    try {
      kernelExecutor.submit(() -> plantModelService.createPlantModel(to, isPersistFromRemote)).get();
    }
