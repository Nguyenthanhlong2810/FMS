...
public class StandardRemoteRouterService
  ...
  public Route getRoute(ClientID clientId, Vehicle vehicle, Point sourcePoint, Point destinationPoint) {
    ...
    return routerService.getRoute(vehicle, sourcePoint, destinationPoint);
  }

  @Override
  public void exportRoute(ClientID clientId, TCSObjectReference<Vehicle> vehicleRef, List<DriveOrder> route) {

    try {
      kernelExecutor.submit(() -> routerService.exportRoute(vehicleRef, route)).get();
    }
  ...
  @Override
  public List<DriveOrder> importRoute(ClientID clientId, TCSObjectReference<Vehicle> vehicleRef) {

    return routerService.importRoute(vehicleRef);
  }
}
