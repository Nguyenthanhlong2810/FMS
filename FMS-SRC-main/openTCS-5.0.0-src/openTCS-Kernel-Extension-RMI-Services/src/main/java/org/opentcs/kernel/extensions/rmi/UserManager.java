...
public class UserManager
   ...
   */
  private final Map<String, UserAccount> knownUsers = new TreeMap<>(String.CASE_INSENSITIVE_ORDER);
  /**
  ...
  public UserAccount getUser(String username, String password) {
    return userAccountProvider.getUserAccount(username,password);
  }
  ...
  public void verifyCredentials(ClientID clientID, UserPermission requiredPermission)
    ...
    if (!checkCredentialsForRole(clientID, requiredPermission)) {
      ResourceBundle bundle = ResourceBundle.getBundle(I18nCommon.BUNDLE_PATH, clientID.getLocale());
      throw new CredentialsException(CredentialsException.Type.PERMISSION_DENIED, bundle.getString("service.permissionDenied"));
    }
  }

  public void changePassword(ClientID clientID, String oldPassword, String newPassword) {
    ClientEntry clientEntry = getClient(clientID);
    if (clientEntry == null) {
      throw new CredentialsException(CredentialsException.Type.PERMISSION_DENIED);
    }
    UserAccount account = knownUsers.get(clientEntry.getUserName());
    if (!account.getPassword().equals(PasswordHash.hash(oldPassword))) {
      throw new CredentialsException(CredentialsException.Type.AUTHENTICATION_FAILED);
    }
    userAccountProvider.changePassword(knownUsers.get(clientEntry.getUserName()), oldPassword, newPassword);
  }
