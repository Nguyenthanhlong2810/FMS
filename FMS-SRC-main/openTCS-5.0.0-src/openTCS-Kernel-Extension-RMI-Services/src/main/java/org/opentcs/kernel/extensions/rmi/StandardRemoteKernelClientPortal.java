...
public class StandardRemoteKernelClientPortal
  ...
  @Override
  public ClientID login(String userName, String password, Locale locale, Predicate<Object> eventFilter)
    ...
    synchronized (userManager.getKnownClients()) {
      ...
      if(account == null){
        account = userManager.getUser(userName, password);
      }
      if (account == null) {
        LOG.debug("Authentication failed for user {}.", userName);
        throw new CredentialsException(CredentialsException.Type.AUTHENTICATION_FAILED, "Authentication failed for user " + userName);
      }

      // Generate a new ID for the client.
      ClientID clientId = new ClientID(userName, locale);
