...
public class RequestResponseMatcher {
  ...
  /**
   * Returns the next request in the queue or an {@link Optional} if none is present.
   *
   * @return The next request in the queue or an {@link Optional} if none is present
   */
  ...
  public void clearRouteRequests() {
    requests.removeIf(request -> request instanceof WorkingRouteRequest);
  }
