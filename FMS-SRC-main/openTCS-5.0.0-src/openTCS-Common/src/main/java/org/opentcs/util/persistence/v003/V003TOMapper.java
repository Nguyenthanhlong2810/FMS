...
/**
 * Provides methods for mapping {@link PlantModelCreationTO} to {@link V003PlantModelTO} and
 * vice versa.
 *
 * @author Martin Grzenia (Fraunhofer IML)
 */
public class V003TOMapper {
   ...
   */
  public PlantModelCreationTO map(V003PlantModelTO model) {
  ...
  /**
   * Maps the given model to a {@link V003PlantModelTO} instance.
   *
   * @param model The model to map.
   * @return The mapped {@link V003PlantModelTO} instance.
   */
  public V003PlantModelTO map(PlantModelCreationTO model) {
    V003PlantModelTO result = new V003PlantModelTO();
    ...
    for (PointTO point : points) {
      result.add(
          new PointCreationTO(point.getName())
              ...
              .withLeftPoint(point.getLeftPoint()).withRightPoint(point.getRightPoint())
    ...
    for (LocationTO location : locations) {
      result.add(
          new LocationCreationTO(location.getName(),
              ...
              .withWidth(location.getWidth())
              .widthHeight(location.getHeight())
      );
    }
      ...
      result.add(
          new GroupCreationTO(group.getName())
              .withMemberNames(group.getMembers().stream()
                  ...
                  .withDescription(group.getDescription()).withId(group.getId())
    ...
    for (PointCreationTO point : points) {
      ...
      pointTO.setxPosition(point.getPosition().getX())
          ...
          .setOutgoingPaths(getOutgoingPaths(point, paths))
              .setLeftPoint(point.getLeftPoint()).setRightPoint(point.getRightPoint())
  ...
  private List<VehicleTO> toVehicleTO(List<VehicleCreationTO> vehicles) {
    ...
    for (VehicleCreationTO vehicle : vehicles) {
      ...
      vehicleTO.setTypeTheme(vehicle.getTypeTheme())
          .setLength((long) vehicle.getLength())
  ...
  private List<LocationTO> toLocationTO(List<LocationCreationTO> locations) {
    ...
    for (LocationCreationTO location : locations) {
      ...
      locationTO.setxPosition(location.getPosition().getX())
          ...
          .setWidth(location.getWidth())
          .setHeight(location.getHeight())
  ...
  private List<GroupTO> toGroupTO(List<GroupCreationTO> groups) {
    ...
    for (GroupCreationTO group : groups) {
      ...
      groupTO.setMembers(toMemberTOs(group.getMemberNames())).setDescription(group.getDescription()).setId(group.getId())
