...
public class ExampleProcessModel
  ...
  private String nextPosition = "0000";
  /**
  ...
  public void setVehicleNextPosition(String nextPosition) {
    String oldValue = this.nextPosition;
    this.nextPosition = nextPosition;
    getPropertyChangeSupport().firePropertyChange(VehicleProcessModel.Attribute.NEXT_POSITION.name(),
                                                  oldValue,
                                                  this.nextPosition);
  }
  ...
  public String getErrorMessageAsString() {
    ...
    getErrorMessage().entrySet().forEach(entry -> {
      builder.append(entry.getKey()).append("|").append(entry.getValue()).append(";");
    });
    return builder.toString();
  }

  public void setVehicleErrorsMessage(String message) {
    setVehicleErrorMessages(splitMessageToEntries(message));
  }

  private HashMap<String, String> splitMessageToEntries(String message) {
    String[] messageEntries = message.split(";");
    messageEntries = Arrays.copyOf(messageEntries, messageEntries.length - 1);
    HashMap<String, String> messages = new HashMap<>();
    for (String entry : messageEntries) {
      String[] pair = entry.split("\\|", 2);
      messages.put(pair[0].trim(), pair[1].trim());
    }
    return messages;
  }

  public void setVehicleIntegrationLevelToRespect() {
    Vehicle.IntegrationLevel oldValue = Vehicle.IntegrationLevel.TO_BE_UTILIZED;

    getPropertyChangeSupport().firePropertyChange(VehicleProcessModel.Attribute.INTEGRATION_LEVEL.name(),
                                                  oldValue,
