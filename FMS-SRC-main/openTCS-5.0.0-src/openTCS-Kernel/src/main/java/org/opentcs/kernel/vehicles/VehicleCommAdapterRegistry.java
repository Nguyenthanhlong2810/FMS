...
public class VehicleCommAdapterRegistry
  ...
  /**
   * The registered validators. Uses a comparator to sort the loopback driver to the end.
   */
  private final Map<VehicleCommAdapterDescription, VehicleCommInfoValidator> validators
          = new HashMap<>();
  /**
  ...
  public VehicleCommAdapterRegistry(LocalKernel kernel, Set<VehicleCommAdapterFactory> factories) {
    ...
    for (VehicleCommAdapterFactory factory : factories) {
      ...
      this.validators.put(factory.getDescription(), factory.getValidator());
    }
  ...
  public VehicleCommInfoValidator findValidatorByName(String name) {
    VehicleCommAdapterDescription description = validators.keySet().stream()
            .filter(d -> d.getClass().getName().equals(name))
            .findFirst()
            .orElse(null);
    if (description != null) {
      return validators.get(description);
    }
