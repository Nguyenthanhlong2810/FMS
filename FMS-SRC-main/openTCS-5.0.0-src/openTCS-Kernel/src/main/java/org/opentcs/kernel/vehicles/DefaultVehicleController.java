...
public class DefaultVehicleController
  ...
  private void handleProcessModelEvent(PropertyChangeEvent evt) {
    ...
    if (Objects.equals(evt.getPropertyName(), VehicleProcessModel.Attribute.POSITION.name())) {
      updateVehiclePosition((String) evt.getNewValue());
                            ...
                            VehicleProcessModel.Attribute.NEXT_POSITION.name())) {
      updateVehicleNextPositionWhenLoop((String) evt.getNewValue());
    }
    else if (Objects.equals(evt.getPropertyName(),
      ...
      updateVehiclePrecisePosition((Triple) evt.getNewValue());
                            ...
                            VehicleProcessModel.Attribute.PRECISE_POSITION_DISTANCE.name())) {
      updateVehiclePrecisePositionByDistance((Double) evt.getNewValue());
    }
    else if (Objects.equals(evt.getPropertyName(),
                            VehicleProcessModel.Attribute.INTEGRATION_LEVEL.name())) {
      updateVehicleIntegrationLevel((Vehicle.IntegrationLevel) evt.getNewValue());
    }
    else if (Objects.equals(evt.getPropertyName(),
                            VehicleProcessModel.Attribute.VOLTAGE.name())) {
      updateVehicleVoltage((Float) evt.getNewValue());
    }
    else if (Objects.equals(evt.getPropertyName(),
                            VehicleProcessModel.Attribute.CURRENT.name())) {
      updateVehicleCurrent((Float) evt.getNewValue());
    }
    else if (Objects.equals(evt.getPropertyName(), VehicleProcessModel.Attribute.IS_WARNING.name())) {
      updateVehicleWarningState((Boolean) evt.getNewValue());
    }
    else if (Objects.equals(evt.getPropertyName(),
                            VehicleProcessModel.Attribute.ERROR_CODE.name())) {
      updateVehicleErrorCode((Integer) evt.getNewValue());
    }
    else if (Objects.equals(evt.getPropertyName(),
            VehicleProcessModel.Attribute.ERROR_MESSAGES.name())) {
      updateVehicleErrorMessages((HashMap<String, String>) evt.getNewValue());
    }
    else if (Objects.equals(evt.getPropertyName(),
      ...
      if (currentDriveOrder != null) {
        vehicleService.updateObjectProperty(currentDriveOrder.getTransportOrder(),
            ...
            VehicleProcessModel.Attribute.SYNC_ROUTES_SUCCESS.name())) {
      updateVehicleCurrentRoute(new WorkingRoute.WorkingRouteRaw((Integer) evt.getNewValue()));
      eventBus.onEvent(new WorkingRouteEvent(WorkingRouteEvent.EventType.SYNC_ROUTES_SUCCESS, vehicle));
    }
    else if (Objects.equals(evt.getPropertyName(),
            VehicleProcessModel.Attribute.SYNC_ROUTES_FAILED.name())) {
      eventBus.onEvent(new WorkingRouteEvent(WorkingRouteEvent.EventType.SYNC_ROUTES_FAILED, vehicle));
    }
    else if (Objects.equals(evt.getPropertyName(),
                            VehicleProcessModel.Attribute.CURRENT_ROUTE.name())) {
      updateVehicleCurrentRoute((WorkingRoute.WorkingRouteRaw) evt.getNewValue());
    }
    else if (Objects.equals(evt.getPropertyName(),
            VehicleProcessModel.Attribute.DUPLICATE_ROUTE.name())) {
      updateVehicleCurrentRoute((WorkingRoute.WorkingRouteRaw) evt.getOldValue());
    }
  }
  ...
  private void updateVehiclePrecisePositionByDistance(double distance)
      throws ObjectUnknownException {
    // Get an up-to-date copy of the vehicle
    Vehicle currVehicle = vehicleService.fetchObject(Vehicle.class, vehicle.getReference());

    if (currVehicle.getIntegrationLevel() != Vehicle.IntegrationLevel.TO_BE_IGNORED) {
      StandardVehicleService service = (StandardVehicleService) vehicleService;
      service.updateVehiclePrecisePositionByDistance(vehicle.getReference(), distance);
    }
  }

  private void updateVehicleNextPositionWhenLoop(String nextPosition)
          throws ObjectUnknownException {
    // Get an up-to-date copy of the vehicle
    Vehicle currVehicle = vehicleService.fetchObject(Vehicle.class, vehicle.getReference());
    ...
    if (currVehicle.getIntegrationLevel() != Vehicle.IntegrationLevel.TO_BE_IGNORED) {
      StandardVehicleService service = (StandardVehicleService) vehicleService;
      service.updateVehicleNextPositionWhenLoop(vehicle.getReference(), nextPosition);
    }
  }

  private void updateVehicleIntegrationLevel(Vehicle.IntegrationLevel integrationLevel) {
    Vehicle currVehicle = vehicleService.fetchObject(Vehicle.class, vehicle.getReference());

    vehicleService.updateVehicleIntegrationLevel(vehicle.getReference(), integrationLevel);
  }

  private void updateVehicleErrorCode(int errorCode) {
    StandardVehicleService service = (StandardVehicleService) vehicleService;
    service.updateVehicleErrorCode(vehicle.getReference(), errorCode);
  }

  private void updateVehicleErrorMessages(HashMap<String, String> errorMessages) {
    StandardVehicleService service = (StandardVehicleService) vehicleService;
    service.updateVehicleErrorMessages(vehicle.getReference(), errorMessages);
  }

  private void updateVehicleVoltage(float voltage)
          throws ObjectUnknownException {
    // Get an up-to-date copy of the vehicle
    Vehicle currVehicle = vehicleService.fetchObject(Vehicle.class, vehicle.getReference());
    ...
    if (currVehicle.getIntegrationLevel() != Vehicle.IntegrationLevel.TO_BE_IGNORED) {
      StandardVehicleService service = (StandardVehicleService) vehicleService;
      service.updateVehicleVoltage(vehicle.getReference(), voltage);
    }
  }

  private void updateVehicleCurrent(float current)
          throws ObjectUnknownException {
    // Get an up-to-date copy of the vehicle
    Vehicle currVehicle = vehicleService.fetchObject(Vehicle.class, vehicle.getReference());
    ...
    if (currVehicle.getIntegrationLevel() != Vehicle.IntegrationLevel.TO_BE_IGNORED) {
      StandardVehicleService service = (StandardVehicleService) vehicleService;
      service.updateVehicleCurrent(vehicle.getReference(), current);
    }
  }

  public void updateVehicleWarningState(boolean isWarning)
          throws ObjectUnknownException {
    vehicleService.updateVehicleWarningState(vehicle.getReference(), isWarning);
  }

  public void updateVehicleCurrentRoute(WorkingRoute.WorkingRouteRaw workingRoute)
    throws ObjectUnknownException {
    StandardVehicleService service = (StandardVehicleService) vehicleService;
    eventBus.onEvent(new WorkingRouteEvent(WorkingRouteEvent.EventType.CURRENT_ROUTE,
            service.updateVehicleCurrentRoute(vehicle.getReference(), workingRoute)));
  }
      ...
      if (curStep.getDestinationPoint().isHaltingPosition()) {
        ...
        futureCommands.add(
            new MovementCommandImpl(orderRoute,
                                    ...
                                    newOrder.getCurrentCountOnSteps() == 0 ? 0 : newOrder.decreaseLoopCount())
        );
      }
    }
  }
  ...
  private boolean canSendNextCommand() {
    ...
    if (waitingForAllocation && futureCommands.peek().getLoopCount() == 0) {
