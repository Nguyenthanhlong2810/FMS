...
import org.opentcs.kernel.periodreport.ReportMail;
...
final class StandardKernel
  ...
  /**
   *The kernel application configuration
   */
  private KernelApplicationConfiguration configuration;
  /**
   * Hibernate ORM Session Factory.
   */
   ...
   *
   * @param eventBus The central event hub to be used.
  ...
  public void initialize() {
    ...
    String db_name = configuration.dbname();
    LOG.info(port);
    LOG.info(username);
    LOG.info(password);
    LOG.info(ip);
    LOG.info(db_name);
    ConnectionPoolCreator.setProperties(port,ip,username,password,db_name);
    //Set kernel name to mail
    ReportMail.name = configuration.name();
  ...
  private void initPeriodicTask() {
    try {
      LOG.debug("Starting period task...");
      ResetableSchedulerTask.getInstance().newTask(new MailingTask(
              ResetableSchedulerTask.getInstance(),
              MailingTask.getPeriodFromDb()
      ));
      LOG.debug("Period task started complete.");
    } catch(Exception e) {
