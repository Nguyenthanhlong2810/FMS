...
public class AttachmentManager
  ...
  public void attachAdapterToVehicle(@Nonnull String vehicleName,
    ...
    objectService.updateObjectProperty(vehicleEntry.getVehicle().getReference(),
                                       ...
                                       factory.getDescription().getClass().getName());
  ...
  public void autoAttachAdapterToVehicle(@Nonnull String vehicleName) {
    ...
    if (factory != null) {
      ...
      if (!factories.isEmpty()) {
        ...
        attachAdapterToVehicle(vehicleName, new NullVehicleCommAdapterFactory());
      }
    }
  }
  ...
  private void initAttachmentPool() {
    vehicleEntryPool.getEntries().forEach(this::createAttachmentInformationForVehicle);
  }

  public void createAttachmentInformationForVehicle(String vehicleName, VehicleEntry entry) {
  ...
  private VehicleCommAdapterFactory findFactoryWithName(@Nullable String name) {
    return commAdapterRegistry.getFactories().stream()
        .filter(factory -> factory.getDescription().getClass().getName().equals(name))
        .findFirst()
        .orElse(null);
  }

  public void createVehicleEntry(Vehicle vehicle) {
    VehicleEntry vehicleEntry = vehicleEntryPool.createVehicleEntry(vehicle);
    createAttachmentInformationForVehicle(vehicle.getName(), vehicleEntry);
    autoAttachAdapterToVehicle(vehicle.getName());
  }

  public void updateVehicleEntry(Vehicle vehicle){
    VehicleEntry vehicleEntry = vehicleEntryPool.getEntryFor(vehicle.getName());
    disableAndTerminateAdapter(vehicleEntry);
    createVehicleEntry(vehicle);
  }

  public void removeVehicleEntry(Vehicle vehicle) {
    VehicleEntry vehicleEntry = vehicleEntryPool.getEntryFor(vehicle.getName());
    disableAndTerminateAdapter(vehicleEntry);
    vehicleEntryPool.getEntries().remove(vehicle.getName());
  }

  public boolean tryValidateVehicleCommInfo(Vehicle vehicle) throws ValidateException {
    String prefAdapter = vehicle.getProperties().get(Vehicle.PREFERRED_ADAPTER);
    VehicleCommAdapterFactory factory = findFactoryWithName(prefAdapter);
    VehicleCommInfoValidator validator = factory.getValidator();
    if (validator == null) {
      return true;
    }

    return validator.validateVehicleCommInfo(vehicle, objectService.fetchObjects(Vehicle.class));
  }
}
