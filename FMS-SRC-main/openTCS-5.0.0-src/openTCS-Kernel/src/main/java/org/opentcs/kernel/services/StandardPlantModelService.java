...
public class StandardPlantModelService
  ...
  public void loadPlantModel()
    ...
    synchronized (globalSyncObject) {
      model.loadVehicleFromDatabase();
      if (!modelPersister.hasSavedModel()) {
        createPlantModel(new PlantModelCreationTO(Kernel.DEFAULT_MODEL_NAME), false);
        return;
      }
  ...
  @Override
  public void createPlantModel(PlantModelCreationTO to, boolean isPersistFromRemote)
    ...
    synchronized (globalSyncObject) {
      ...
      if (isPersistFromRemote) {
        model.createModelInfoFromDatabase();
      }
    }
