...
public class StatisticsEventLogger
  ...
  //Them,sua thong tin vao database
  PsqlLogging logging = new PsqlLogging();
  private void handlingTransportOrder(StatisticsEvent event,TransportOrder ordernaw){
    try {
      LOG.info(event.toString());
      switch (event){
        case ORDER_ACTIVATED :
          logging.insertTO(ordernaw);
          break;
        case ORDER_ASSIGNED:
          logging.updateTOassign(ordernaw);
          break;
        case ORDER_FINISHED_SUCC:
          logging.updateTOfinish(ordernaw, true);
          logging.insertDRO(ordernaw);
          break;
        case ORDER_CROSSED_DEADLINE:
          logging.updateTOcrossdeadline(ordernaw);
          break;
        case ORDER_FINISHED_FAIL:
          logging.updateTOfinish(ordernaw, false);
          logging.insertDRO(ordernaw);
          break;
      }
    }catch (Exception UwU){UwU.printStackTrace();}
  }
  private void handlingVehicle(StatisticsEvent event, Vehicle vehiclenaw){
    try {
      LOG.info(event.toString());
      switch (event)
      {
        case VEHICLE_STARTS_PROCESSING:
          logging.vehicleInsert(vehiclenaw,StatisticsEvent.VEHICLE_STARTS_PROCESSING);
          break;
        case VEHICLE_STOPS_PROCESSING:
          logging.vehicleInsert(vehiclenaw,StatisticsEvent.VEHICLE_STOPS_PROCESSING);
          break;
        case VEHICLE_STARTS_CHARGING:
          logging.vehicleInsert(vehiclenaw,StatisticsEvent.VEHICLE_STARTS_CHARGING);
          break;
        case VEHICLE_STOPS_CHARGING:
          logging.vehicleInsert(vehiclenaw,StatisticsEvent.VEHICLE_STOPS_CHARGING);
          break;
        case VEHICLE_STARTS_WAITING:
          logging.vehicleInsert(vehiclenaw,StatisticsEvent.VEHICLE_STARTS_WAITING);
          break;
        case VEHICLE_STOPS_WAITING:
          logging.vehicleInsert(vehiclenaw,StatisticsEvent.VEHICLE_STOPS_WAITING);
          break;
      }
  ...
  private void processOrderEvent(TCSObjectEvent event) {
    ...
    if (orderNow.hasState(TransportOrder.State.ACTIVE)
      ...
      handlingTransportOrder(StatisticsEvent.ORDER_ACTIVATED , orderNow);
    }
    ...
    if (orderNow.hasState(TransportOrder.State.BEING_PROCESSED)
      ...
      handlingTransportOrder(StatisticsEvent.ORDER_ASSIGNED , orderNow);
    }
    ...
    if (orderNow.hasState(TransportOrder.State.FINISHED)
      ...
      handlingTransportOrder(StatisticsEvent.ORDER_FINISHED_SUCC , orderNow);
      ...
      if (orderNow.getFinishedTime().isAfter(orderNow.getDeadline())) {
        ...
        handlingTransportOrder(StatisticsEvent.ORDER_CROSSED_DEADLINE , orderNow);
      }
    }
    ...
    if (orderNow.hasState(TransportOrder.State.FAILED)
      ...
      handlingTransportOrder(StatisticsEvent.ORDER_FINISHED_FAIL , orderNow);
    }
  }
  /**
  ...
  private void processVehicleEvent(TCSObjectEvent event) {
    ...
    if (vehicleNow.getTransportOrder() != null && vehicleOld.getTransportOrder() == null) {
      ...
      handlingVehicle(StatisticsEvent.VEHICLE_STARTS_PROCESSING,vehicleNow);
    }
    // Did the vehicle finish a transport order?
    if (vehicleNow.getTransportOrder() == null && vehicleOld.getTransportOrder() != null) {
      writeEvent(StatisticsEvent.VEHICLE_STOPS_PROCESSING, vehicleNow.getName());
      handlingVehicle(StatisticsEvent.VEHICLE_STOPS_PROCESSING,vehicleNow);
    }
    ...
    if (vehicleNow.hasState(Vehicle.State.CHARGING)
      ...
      handlingVehicle(StatisticsEvent.VEHICLE_STARTS_CHARGING,vehicleNow);
    }
    // Did the vehicle start charging?
    if (!vehicleNow.hasState(Vehicle.State.CHARGING)
        && vehicleOld.hasState(Vehicle.State.CHARGING)) {
      writeEvent(StatisticsEvent.VEHICLE_STOPS_CHARGING, vehicleNow.getName());
      handlingVehicle(StatisticsEvent.VEHICLE_STOPS_CHARGING,vehicleNow);
    }
    ...
    if (vehicleNow.hasProcState(Vehicle.ProcState.PROCESSING_ORDER)
      ...
      handlingVehicle(StatisticsEvent.VEHICLE_STARTS_WAITING,vehicleNow);
    }
    ...
    if (vehicleNow.hasProcState(Vehicle.ProcState.PROCESSING_ORDER)
      ...
      handlingVehicle(StatisticsEvent.VEHICLE_STOPS_WAITING,vehicleNow);
    }
    if (!vehicleNow.getErrorMessages().equals(vehicleOld.getErrorMessages()))
    {
      logging.insertErrorlog(vehicleNow, vehicleOld);
    }


    //Vehicle iz Stand still
    if (vehicleOld.hasState(Vehicle.State.EXECUTING) && isVehicleStop(vehicleNow)||
            vehicleOld.hasState(Vehicle.State.WARNING) && isVehicleStop(vehicleNow)){
      //logging
      logging.updateVehicleLogStop(vehicleNow);
    }
    //vehicle is running
    if (isVehicleStop(vehicleOld) && vehicleNow.hasState(Vehicle.State.EXECUTING)||
            isVehicleStop(vehicleOld) && vehicleNow.hasState(Vehicle.State.WARNING)){
      //logging
      logging.insertVehicleLogStart(vehicleNow);
    }
  }
  /**
  ...
  private boolean isVehicleStop(Vehicle vehicle){
    return vehicle.hasState(Vehicle.State.UNKNOWN)
            || vehicle.hasState(Vehicle.State.ERROR)
            || vehicle.hasState(Vehicle.State.IDLE)
            || vehicle.hasState(Vehicle.State.WARNING);
  }
